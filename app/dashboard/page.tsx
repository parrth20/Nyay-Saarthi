...
import {
    FileText, Upload, Search, Inbox, MessageSquare, AlertTriangle, User as UserIcon, Clock, Briefcase, FileSignature, Bot, CheckCircle // <-- Added CheckCircle
...
export default async function Dashboard() {
  ...
  const fetchLastChat = async (): Promise<Chat | null> => {
      ...
      if (chatError && chatError.code !== 'PGRST116') { // Ignore "No rows found"
  ...
  const calculateStats = (docs: Document[]) => {
      ...
      const analyzedCount = docs.filter(d => analyzedStatuses.includes(d.status?.toLowerCase() ?? '')).length; // Check against array
      const processingCount = docs.filter(d => processingStatuses.includes(d.status?.toLowerCase() ?? '')).length; // Check against array
      const requiresAttentionCount = docs.filter(d => errorStatuses.includes(d.status?.toLowerCase() ?? '')).length; // Check against array

      // Define stats array with correct icon references
      return [
          { label: "कुल दस्तावेज़", value: totalDocs, IconComponent: FileText, color: "text-blue-600 bg-blue-100" }, // Use IconComponent alias
          { label: "विश्लेषित", value: analyzedCount, IconComponent: CheckCircle, color: "text-green-600 bg-green-100" }, // Use IconComponent alias
          { label: "प्रगति में", value: processingCount, IconComponent: Clock, color: "text-yellow-600 bg-yellow-100" }, // Use IconComponent alias
          { label: "ध्यान दें", value: requiresAttentionCount, IconComponent: AlertTriangle, color: "text-red-600 bg-red-100" }, // Use IconComponent alias
      ];
  };
  ...
  const quickActions = [
      { label: "नया दस्तावेज़ अपलोड करें", href: "/upload", IconComponent: Upload }, // Use IconComponent alias
      { label: "AI सहायक से पूछें", href: "/chat", IconComponent: MessageSquare }, // Use IconComponent alias
      { label: "परामर्श बुक करें", href: "/consultation", IconComponent: Briefcase }, // Use IconComponent alias
  ];
  // ---
      ...
      <div className="max-w-7xl mx-auto space-y-8">
        ...
        <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4 mb-6">
          <div className="flex items-center gap-4">
              ...
              <div>
                <h1 className="text-3xl md:text-4xl font-bold text-gray-900 mb-0.5">स्वागत है, {displayName}!</h1>
                <p className="text-gray-600 text-lg">यहां आपके कानूनी दस्तावेज़ों का अवलोकन है।</p>
              </div>
          </div>
          <div className="flex w-full md:w-auto items-center gap-3 mt-2 md:mt-0">
            ...
            <div className="relative flex-grow md:flex-grow-0">
             ...
             {/* Upload Button */}
            <Link href="/upload">
              <Button size="lg" className="rounded-full bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-700 hover:to-indigo-700 text-white font-semibold shadow-md h-11 px-6 whitespace-nowrap transition-transform transform hover:scale-105">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          {stats.map((stat, index) => (
            ...
            <Card key={index} className="shadow-lg border-0 rounded-xl overflow-hidden bg-white hover:shadow-xl transition-shadow duration-300 transform hover:-translate-y-1">
              <CardContent className="p-5 flex items-center gap-4">
                 <div className={`p-3 rounded-full ${stat.color} flex-shrink-0 shadow-inner`}>
                    <stat.IconComponent className="h-6 w-6" /> {/* Use IconComponent alias */}
                 </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <Card className="lg:col-span-2 shadow-lg border-0 rounded-xl bg-white overflow-hidden">
            <CardHeader className="border-b border-gray-100 bg-gray-50/50">
              ...
              <CardDescription className="text-sm mt-1">आपके द्वारा हाल ही में एक्सेस किए गए या अपलोड किए गए दस्तावेज़।</CardDescription>
            </CardHeader>
            <CardContent className="p-0">
              {documents.length === 0 ? (
                ...
                <div className="text-center py-16 px-6 flex flex-col items-center">
                  ...
                  <h3 className="text-xl font-semibold text-gray-700 mb-2">आपका डैशबोर्ड खाली है</h3>
                  <p className="text-gray-500 mb-6 max-w-sm mx-auto">विश्लेषण और सरलीकरण शुरू करने के लिए अपना पहला कानूनी दस्तावेज़ अपलोड करें।</p>
                  <Link href="/upload">
                    <Button size="lg" className="rounded-full bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-700 hover:to-indigo-700 text-white font-semibold shadow-md px-8 py-3 transition-transform transform hover:scale-105">
                      <Upload className="h-5 w-5 mr-2" />
                ...
                <div className="divide-y divide-gray-100 max-h-[400px] overflow-y-auto">
                  {documents.map((doc) => {
                     ...
                     const showSummaryButton = analyzedStatuses.includes(doc.status?.toLowerCase() ?? ''); // Use the array check
                        ...
                        <div className="flex items-center gap-4 flex-grow overflow-hidden">
                          {/* Icon based on type */}
                           <div className="p-2 bg-gray-100 rounded-lg flex-shrink-0">
                             {doc.name?.toLowerCase().includes('समझौता') || doc.name?.toLowerCase().includes('contract') ? <FileSignature className="h-5 w-5 text-indigo-600" /> : <FileText className="h-5 w-5 text-gray-500" />}
                           </div>
                          <div className="overflow-hidden">
                            <h4 className="font-semibold text-gray-800 truncate text-base group-hover:text-blue-700 transition-colors" title={doc.name ?? 'Untitled'}>
                            ...
                            <div className="flex items-center gap-2 text-xs text-gray-500 mt-1 flex-wrap">
                              ...
                              <span title={new Date(doc.created_at).toLocaleString()}>
                                  {formatDistanceToNow(new Date(doc.created_at), { addSuffix: true, locale: hi })} {/* Hindi Locale */}
                              </span>
                            </div>
                          </div>
                        </div>
             ...
             {documents.length > 0 && (
                 <div className="p-4 text-center border-t border-gray-100 bg-gray-50/50">
                     <Button variant="link" className="text-sm text-blue-600 hover:text-blue-800">सभी दस्तावेज़ देखें ({documents.length})</Button>
                 </div>
               )}
          </Card>
            ...
            <Card className="shadow-lg border-0 rounded-xl bg-white overflow-hidden">
                ...
                <CardContent className="p-5">
                    {lastChat?.last_message ? (
                        <div>
                            <p className="text-gray-700 text-sm mb-3 line-clamp-4 leading-relaxed"> {/* Added leading-relaxed */}
                                "{lastChat.last_message}"
                            </p>
                            <div className="flex justify-between items-center mt-4"> {/* Added mt-4 */}
                                <p className="text-xs text-gray-500">
                                    {lastChat.updated_at ? formatDistanceToNow(new Date(lastChat.updated_at), { addSuffix: true, locale: hi }) : 'Unknown time'}
                                </p>
                                <Link href="/chat">
                        ...
                        <div className="text-center py-6 text-gray-500">
                            ...
                            <Link href="/chat">
                                <Button variant="link" size="sm" className="mt-2 text-blue-600 hover:text-blue-800 font-medium">AI से पूछना शुरू करें</Button>
                            </Link>
                        </div>
                    )}
                </CardContent>
              ...
              <CardContent className="space-y-3">
                {quickActions.map((action) => (
                  ...
                  <Link href={action.href} key={action.label} className="block">
                    ...
                    >
                      <action.IconComponent className="h-5 w-5 mr-3 text-gray-500 group-hover:text-blue-600 transition-colors" /> {/* Use IconComponent alias and add hover effect */}
                      {action.label}
                    </Button>
                  </Link>
                ))}
